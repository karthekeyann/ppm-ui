...
(function($) {
  ...
  function Plugin(element, options) {
    ...
    function init() {
      ...
      if (options.wrapContent) {
        $contentEl.wrap('<div class="tse-scroll-content" style="width:auto;" />');
      }
  ...
  var ff;
    $.myplugin = function () { }
    $.myplugin.CallMe3 = function (receivedEve) {
      if (receivedEve == "next") {
        count = 1;
      } else if (receivedEve == "prev") {
        count = 2;
      } else {
        count = 3;
        if(receivedEve == 0){
    ...
    function resizeScrollbar() {
      ...
      if (scrollbarSize < contentSize) {
        if (scrollDirection === 'vert'){
          if(count == 1){
            scrollVal+=52;
            $scrollContentEl[scrollOffsetAttr](scrollVal);
            count = 0;
          } else if(count == 2){
            if(scrollVal == 0){
              scrollVal = ff-52;
            ...
            ff = clickedLi*52;
            $scrollContentEl[scrollOffsetAttr](ff);
            count = 0;
          }
          else {
